...
export const generateAwardDrafts = action({
  ...
  handler: async (ctx, args) => {
    ...
    for (const supplier of suppliers) {
      ...
      await ctx.runMutation(internal.awards.insertAwardDraft, { projectId: args.projectId, supplierId: supplier._id, awardId: args.awardId, kind: isWinner ? "award" : "decline", subject, body, attachmentId });
    }
    return null;
  },
});