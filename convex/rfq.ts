...
export const draftRfqs = action({
  ...
  handler:async(ctx,args)=>{
    ...
    for(const supplier of selected){
      ...
      const matched=lineItems.filter((item: Doc<"lineItems">)=>cats.has(norm(item.category)));
      if(!matched.length) continue;
      const body=buildBody(project.name,project.location,supplier.name,deadline,matched);
      await ctx.runMutation(internal.drafts.insertDraft,{projectId:args.projectId,supplierId:supplier._id,kind:"rfq",subject:`Request for Quote: ${project.name}`,body,rfqLineItemIds:matched.map((item: Doc<"lineItems">)=>item._id)});
    }
    return null;
  }
});
