  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => void handleDiscover()} disabled={discovering} className="rounded-md bg-neutral-100 text-neutral-900 px-3 py-1.5 text-sm font-medium disabled:opacity-50">{discovering ? 'Discovering…' : 'Discover suppliers'}</button>
      </div>
      {error && <p className="text-sm text-red-400">{error}</p>}
      <label className="flex items-center gap-2 text-xs text-neutral-400 cursor-pointer w-fit">
        <input type="checkbox" checked={project.autoApproveFollowUps ?? false} onChange={(e) => void setAutoApprove({ projectId: project._id, enabled: e.currentTarget.checked })} />
