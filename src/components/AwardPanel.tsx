...
export function AwardPanel({ project }: { project: Doc<'projects'> }) {
  ...
  const selectedRows = mode === 'single' && supplierId ? preview.supplierRows.filter(r => r.supplierId === supplierId) : preview.rows
  const selectedMissing = mode === 'single' && supplierId ? preview.missingBySupplier.find(s => s.supplierId === supplierId)?.missingLineItemIds.length ?? 0 : Math.max(0, preview.scopeLineItemIds.length - preview.rows.length)
  const ready = preview.scopeLineItemIds.length > 0 && selectedMissing === 0 && !!address.trim() && !!date && (mode === 'split' || !!supplierId)
  ...
  async function confirmAward() {
    ...
    try {
      ...
      const id = await award({ projectId: project._id, mode, supplierId: mode === 'single' ? supplierId as Id<'suppliers'> : undefined, deliveryAddress: address, deliveryDate: date, quantities: preview.scopeLineItemIds.map(lineItemId => ({ lineItemId, quantity: quantities[lineItemId as string] ?? selectedRows.find(r => r.lineItemId === lineItemId)?.quantity ?? 0 })) })
  ...
  return <div className="space-y-4 border-t border-white/10 pt-5">
    ...
    <div className="grid gap-2 sm:grid-cols-3"><div className={`rounded-xl border p-3 ${preview.scopeLineItemIds.length ? 'border-emerald-500/20 bg-emerald-500/[.04]' : 'border-red-500/20 bg-red-500/[.04]'}`}><p className="text-[11px] uppercase tracking-wider text-neutral-500">RFQ scope</p><p className="mt-1 text-sm font-semibold">{preview.scopeLineItemIds.length} items</p></div><div className={`rounded-xl border p-3 ${selectedMissing === 0 ? 'border-emerald-500/20 bg-emerald-500/[.04]' : 'border-amber-500/20 bg-amber-500/[.04]'}`}><p className="text-[11px] uppercase tracking-wider text-neutral-500">Quote coverage</p><p className="mt-1 text-sm font-semibold">{Math.max(0, preview.scopeLineItemIds.length - selectedMissing)} / {preview.scopeLineItemIds.length}</p></div><div className={`rounded-xl border p-3 ${address.trim() && date ? 'border-emerald-500/20 bg-emerald-500/[.04]' : 'border-amber-500/20 bg-amber-500/[.04]'}`}><p className="text-[11px] uppercase tracking-wider text-neutral-500">Delivery details</p><p className="mt-1 text-sm font-semibold">{address.trim() && date ? 'Ready' : 'Missing'}</p></div></div>
    ...
    {mode === 'single' && <select value={supplierId} onChange={e => setSupplierId(e.target.value as Id<'suppliers'>)} className="bg-neutral-900 border border-white/10 rounded-xl px-3 py-2.5 text-sm w-full"><option value="">Choose supplier…</option>{preview.suppliers.map(s => <option key={s.supplierId} value={s.supplierId}>{s.supplierName}</option>)}</select>}
    ...
    <div className="rounded-xl border border-white/10 divide-y divide-white/[.05]">{selectedRows.map(r => <div key={r.lineItemId} className="p-3 flex items-center gap-3 text-sm"><span className="flex-1">{r.name}<span className="block text-xs text-neutral-600">{r.supplierName} · {r.unitPrice.toLocaleString()} {project.currency}/{r.unit}</span></span><input type="number" min="0" step="any" value={quantities[r.lineItemId as string] ?? r.quantity} onChange={e => setQuantities(q => ({ ...q, [r.lineItemId as string]: Number(e.target.value) }))} className="w-24 bg-neutral-900 border border-white/10 rounded-lg px-2 py-1 text-right"/><span className="text-neutral-500 w-24 text-right">{(r.unitPrice * (quantities[r.lineItemId as string] ?? r.quantity)).toLocaleString()} {project.currency}</span></div>)}</div>
